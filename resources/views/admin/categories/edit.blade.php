...
<div class="py-12 h-screen">
    ...
    <div class="flex flex-col items-center justify-center ">
        ...
        <form enctype="multipart/form-data" class="space-y-6 bg-gray-800 p-8 rounded-lg shadow-lg " style="width: 500px;" method="POST" action="{{route("admin.categories.update",$category->id)}}">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-sm font-medium text-gray-300">Name</label>
                <div class="mt-1">
                    <input type="text" id="name"  name="name" value="{{$category->name}}" class="block w-full bg-gray-700 border-gray-500 focus:ring-indigo-500 focus:border-indigo-500 rounded-md shadow-sm py-2 px-3 text-sm text-gray-100" />
                    @error('name')
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea id="description"  name='description' value=""
                     rows="3"  class="block w-full bg-gray-700 border-gray-500 focus:ring-indigo-500 focus:border-indigo-500 rounded-md shadow-sm py-2 px-3 text-sm text-gray-100">{{$category->description}}</textarea>
                    @error('description')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </div>
